...
def get_top_entry_dfs(entry):
    ...
    for event in range(1, current_event + 1):
        try:
            entry_season_request = requests.get(f'https://fantasy.premierleague.com/api/entry/{entry}/event/{event}/picks/')
            ...
            logger.error(f'Unable to get data for entry {entry} for event {event} with exception {e}')
            continue
