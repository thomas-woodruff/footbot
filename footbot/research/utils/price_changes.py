...
def get_player_value(element_all, elements_df):
    """
    ...
    :param elements_df: Dataframe including values of players
    :return: Value of player
    """

    try:
        value = elements_df.loc[
            elements_df["element_all"] == element_all, "value"
        ].iloc[0]
    except IndexError:
        raise Exception("Player missing from `elements_df`")

    return value


def update_player_value(element_all, updated_value, elements_df):
    """
    ...
    :param elements_df: Dataframe including values of players
    :return: None - updates `elements_df` in place
    """

    # check player is not missing from element data dataframe
    get_player_value(element_all, elements_df)

    elements_df.loc[elements_df["element_all"] == element_all, "value"] = updated_value
...
def update_purchase_prices(squad, purchase_price_dict, elements_df):
    """
    ...
    :param elements_df: Dataframe including values of all players
    ...
    for element_all in squad:
        if element_all not in purchase_price_dict.keys():
            current_price = get_player_value(element_all, elements_df)
...
def update_player_values_with_selling_prices(elements_df, purchase_price_dict):
    """
    ...
    for element_all, purchase_price in purchase_price_dict.items():
        ...
        update_player_value(element_all, selling_price, elements_df)


def calculate_team_value(squad, elements_df):
    """
    ...
    return np.sum([get_player_value(e, elements_df) for e in squad])
