def calculate_points(
    ...
    first_team_points = sum(
        [i["total_points"] for i in first_team if not i["is_captain"]]
    )
    ...
    captain_points = sum(
        [i["total_points"] * captain_multiplier for i in first_team if i["is_captain"]]
    )

    if bench_boost:
        bench_points = sum([i["total_points"] for i in bench])
    else:
        bench_points = 0
